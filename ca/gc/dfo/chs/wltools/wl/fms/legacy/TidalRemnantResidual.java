...
final public class TidalRemnantResidual extends LegacyResidual implements ILegacyFMSResidual {
   ...
   */
  public TidalRemnantResidual(@NotNull final Residual residualCfg,
                              @NotNull final TidalRemnant tidalRemnantCfg, @Min(1) final Double timIncrDtMinutes,
                              @NotNull final String stationId) {
    
    super(residualCfg, stationId);
    
    this.log.debug("TidalRemnantResidual constr. Start");
    
    this.trData = new TidalRemnantData(tidalRemnantCfg, timIncrDtMinutes.doubleValue());
    
    this.log.debug("TidalRemnantResidual constr.  End");
  }
  
  /**
   * Estimate(in fact compute, estimate is used for the polymorphic OO aspect) tidal remnant ocmponent at a
  ...
  public final WLStationTimeNode estimate(@NotNull final WLStationTimeNode wlStationTimeNode, final boolean apply) {
    ...
    this.log.debug("TidalRemnantResidual estimate: start, apply=" + apply);
    ...
    if (nts < this.lastUpdateSse.seconds()) {
      ...
      this.log.error("TidalRemnantResidual estimate: nts<this.lastUpdateSse.seconds() ! nts must be larger or equal " +
          "to this.lastUpdateSse.seconds()");
      throw new RuntimeException("TidalRemnantResidual estimate");
    }
    
    final double timeOffset = (double) (nts - this.lastUpdateSse.seconds());
    
    this.log.debug("TidalRemnantResidual estimate: timeOffset=" + timeOffset);
...
//        this.log.debug("TidalRemnantResidual estimate: trd.dt="+ trd.dt);
    ...
    trn.remnant().set(trd.computeRemnant(), Math.sqrt(1.0 - trd.getErrorWeight(timeOffset * trd.tauInv)) * trd.eps);
    
    this.log.debug("TidalRemnantResidual estimate: trn.remnant.getZw()=" + trn.remnant.getZw());
    this.log.debug("TidalRemnantResidual estimate: trn.remnant.getError()=" + trn.remnant.getError());
    ...
    this.log.debug("TidalRemnantResidual estimate: end");
  ...
  public final WLStationTimeNode getFMSTimeNode(final WLStationTimeNode pstrWLStationTimeNode,
    ...
    this.log.debug("TidalRemnantResidual getFMSTimeNode: sse dt=" + secondsSinceEpoch.dateTimeString(true));
    this.log.debug("TidalRemnantResidual getFMSTimeNode: pstr=" + pstrWLStationTimeNode);
  ...
  public final long setup(@Min(0) final long lastWLOSse,
    ...
    this.log.debug("TidalRemnantResidual setup Start: superSseStart dt=" + SecondsSinceEpoch.dtFmtString(superSseStart));
    
    this.initStats(predictionsMeasurementsList);
    
    this.log.debug("TidalRemnantResidual setup End");
  ...
  protected final LegacyResidual initStats(@NotNull final List<MeasurementCustom> predictionMeasurementsList) {
    ...
    this.log.debug("TidalRemnantResidual initStats Start");
    
    this.trData.initInvXpx(predictionMeasurementsList);
    
    this.log.debug("TidalRemnantResidual initStats End");
  ...
  public final WLStationTimeNode update(@NotNull final WLStationTimeNode wlStationTimeNode) {
    ...
    this.log.debug("TidalRemnantResidual update: wlstn=" + wlStationTimeNode);
    this.log.debug("TidalRemnantResidual update: wlstn dt=" + wlStationTimeNode.getSse().dateTimeString(true));
    this.log.debug("TidalRemnantResidual update: trd.tau=" + trd.tau);
    this.log.debug("TidalRemnantResidual update: trd.dt=" + trd.dt);
    ...
    this.log.debug("TidalRemnantResidual update: trd.beta=" + trd.beta);
    ...
    this.log.debug("TidalRemnantResidual update: trd.eps1=" + trd.eps1);
    this.log.debug("TidalRemnantResidual update: trd.eps2=" + trd.eps2);
    this.log.debug("TidalRemnantResidual update: trd.k1=" + trd.k1);
    ...
    this.log.debug("TidalRemnantResidual update: newRemnantValue=" + newRemnantValue);
