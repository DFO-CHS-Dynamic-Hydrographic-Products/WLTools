...
//    }
  ...
  /**
   * @param tidalRemnantCfg : TidalRemnant config. object obtained from the same(its a MUST) WL station
   *                        ForecastingContext object.
   * @param timeIncrMinutes : The time increment in minutes used for the tidal remnant computations.
   */
  public TidalRemnantData(final TidalRemnant tidalRemnantCfg, final double timeIncrMinutes) {
   ...
   * @param timeIncrMinutes : The time increment in minutes used for the tidal remnant computations.
   */
  public TidalRemnantData(final double tauHours, final double dtMinutes, final double eps1Max, final double eps2Max,
                          final double timeIncrMinutes) {
    
    super(0, tauHours, dtMinutes);
    
    this.log.debug("TidalRemnantData constr.: tauHours=" + tauHours);
    this.log.debug("TidalRemnantData constr.: dtMinutes=" + dtMinutes);
    this.log.debug("TidalRemnantData constr.: eps1Max=" + eps1Max);
    this.log.debug("TidalRemnantData constr.: eps2Max=" + eps2Max);
    this.log.debug("TidalRemnantData constr.: timeIncrMinutes=" + timeIncrMinutes);
  ...
  private void init(final double eps1Max, final double eps2Max, final double timeIncrMinutes) {
    ...
    if (eps1Max <= 0.0) {
      ...
      this.log.error("TidalRemnantData init: eps1Max <= 0.0 !");
      throw new RuntimeException("TidalRemnantData init");
    }
    
    if (eps2Max <= 0.0) {
      
      this.log.error("TidalRemnantData init: eps2Max <= 0.0 !");
      throw new RuntimeException("TidalRemnantData init");
    }
   ...
   */
  protected final TidalRemnantData initInvXpx(@NotNull @Size(min = 1) final List<MeasurementCustom> predictionMeasurementsList) {
    ...
    if (nbWlpNeeded > predictionMeasurementsList.size()) {
      this.log.warn("nbWlpNeeded > wlpa.size() !");
