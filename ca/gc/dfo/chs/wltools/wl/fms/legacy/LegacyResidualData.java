...
public class LegacyResidualData implements ILegacyFMS {
   ...
   * (See src file itself for more detailed comments)
   */
...
//    }
   ...
   */
  public LegacyResidualData(@Min(0) final int nbAuxCov, @Min(1) final double tauHours, @Min(1) final double dtMinutes) {
    
    this.log.debug("LegacyResidualData constructor: tauHours=" + tauHours);
    this.log.debug("LegacyResidualData constructor: dtMinutes=" + dtMinutes);
    
    if (tauHours > MEMORY_TIME_SCALE_HOURS_MAX) {
      
      this.log.error("LegacyResidualData constructor: tauHours > MEMORY_TIME_SCALE_HOURS_MAX !");
...
//        }
    ...
    if (dtMinutes > DELTA_T_MINUTES_MAX) {
      ...
      this.log.error("LegacyResidualData constructor: dtMinutes > DELTA_T_MINUTES_MAX !");
      throw new RuntimeException("LegacyResidualData constructor.");
    }
...
//        }
    ...
    //--- Avoid re-computing the same costly division a million times in heavy loops.
    this.alphaInv = 1.0 / this.alpha;
    
    //--- Avoid re-computing the same costly mult. a million times in heavy loops.
    ...
    this.log.debug("LegacyResidualData constructor: this.tau=" + this.tau);
    this.log.debug("LegacyResidualData constructor: this.dt=" + this.dt);
    this.log.debug("LegacyResidualData constructor: this.alpha=" + this.alpha);
    
    if (nbAuxCov > 0) {
      
      this.log.debug("LegacyResidualData constructor: Allocating numbercrunching data size->" + nbAuxCov);
      this.allocInit(nbAuxCov);
      
    } else {
      this.log.debug("LegacyResidualData constructor: Allocating numbercrunching data deferred to child class");
    }
  }
  ...
  protected final double dataTimeScaling() {
    ...
    this.log.debug("LegacyResidualData dataTimeScaling: omega=" + omega);
    this.log.debug("LegacyResidualData dataTimeScaling: omega2=" + omega2);
  ...
  protected final double getErrorWeight(@Min(0) final double timeFactor) {
    ...
    this.log.debug("LegacyResidualData getErrorWeight:  this.omega2=" + this.omega2);
  ...
  protected final double getValueWeight(@Min(0) final double timeFactor) {
    ...
    this.log.debug("LegacyResidualData getValueWeight:  this.omega=" + this.omega);
  ...
  protected final D1Data OLSRegression(final double zY, @NotNull final D1Data beta) {
    ...
    this.log.debug("LegacyResidualData OLSRegression: bef. update this.invXpX=" + this.invXpX.toString());
    
    this.updateInvXpx();
    
    this.log.debug("LegacyResidualData OLSRegression: aft update this.invXpX=" + this.invXpX.toString());
    this.log.debug("LegacyResidualData OLSRegression: zY=" + zY);
    this.log.debug("LegacyResidualData OLSRegression: this.zX=" + this.zX);
...
//        }
    ...
    this.log.debug("LegacyResidualData OLSRegression: this.xpY=" + this.xpY.toString());
  ...
  protected final LegacyResidualData updateInvXpx() {
    ...
    this.log.debug("LegacyResidualData updateInvXpx: mx=" + mx.toString());
    ...
    this.log.debug("LegacyResidualData updateInvXpx: denomInv=" + denomInv);
    ...
    this.invXpX.subtract(this.mxxtM.colD1xRowD1(mx, zX.rowD1xD2(this.invXpX)).multWith(denomInv));
...
//    }
  ...
  protected final LegacyResidualData checkNumberCrunchingSize(final int finalSize) {
    ...
    this.log.debug("LegacyResidualData checkNumberCrunchingSize: start, initialSize=" + initialSize + ", finalSize=" + finalSize);
    
    if (initialSize < finalSize) {
      
      this.log.error("LegacyResidualData checkNumberCrunchingSize: finalSize must be less or equal to initialSize ! " +
          "initialSize=" +
      ...
      this.log.warn("LegacyResidualData checkNumberCrunchingSize: Need to downsize numbercrunching data size from " +
          initialSize + " to " + finalSize);
      
      this.allocInit(finalSize);
      
    } else {
      this.log.debug("LegacyResidualData checkNumberCrunchingSize: No need to downsize numbercrunching data! ");
    }
    
    this.log.debug("LegacyResidualData checkNumberCrunchingSize: end.");
